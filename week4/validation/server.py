...
from aiohttp_apispec import docs, querystring_schema, response_schema, \
    validation_middleware, json_schema
...
print(
    'настроили aiohttp_apispec ДО настройки routes. Путь для SwaggerUI = /swagger, для json-Swagger = /swagger.json')
setup_aiohttp_apispec(app, title='Documentation', swagger_path='/swagger',
                      url='/swagger.json')


class RequestSchema(Schema):
    first_name = fields.String(required=True)
    height = fields.Float(validate=validate.Range(min=150, max=210),
                          load_default=170)
...
class UserView(web.View):
    @docs(tags=['users'],
          summary='Получает информацию о пользователе и валидирует ее')
...
class TestView(web.View):
    @docs(tags=['users'],
          summary='Нужен для демонстрации того, как Swagger отобразит этот View')
...
print(
    'добавили Route /test_route, чтобы показать как он отобразится в Swagger-документации')
