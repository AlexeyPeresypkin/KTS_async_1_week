...
async def second_middleware(request: web.Request, handler):
    ...
    print(
        f'Application.second_middleware: следующим запрос будет обрабатывать {handler}')

    request['second_middleware'] = 'important_data'
    print(
        f'Application.second_middleware: добавила в объект запроса такие данные: {request["second_middleware"]}')
...
async def first_middleware(request: web.Request, handler):
    ...
    print(
        f'Application.first_middleware: следующий запрос будет обрабатывать {handler}')
    resp = await handler(request)
...
print(
    'Application: добавили сначала first_middleware, а потом second_middleware')
