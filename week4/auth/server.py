...
class UnsecuredView(web.View):
    ...
    async def get(self):
        ...
        return web.Response(
            text='данные доступные для неавторизованного пользователя')
...
class SecuredView(web.View):
    ...
    async def get(self):
        session = await get_session(
            self.request)  # пытаемся загрузить объект сессии из куки
        ...
        print(
            f'SecuredView: пришел пользователь {session["user"]["username"]}, '
            f'последний раз он логинился {session["user"]["last_login"]}')
        return web.Response(
            text='данные доступные только для авторизованного пользователя')
...
class AuthView(web.View):
    ...
    async def get(self):
        ...
        print(
            f'AuthView: теперь в сессии хранится объект пользователя: {session["user"]=}')
